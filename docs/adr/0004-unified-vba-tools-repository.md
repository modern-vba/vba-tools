...
test execution stay inside `vba-dev`. The VS Code extension invokes the CLI
and maps its output into VS Code UI surfaces; the language server stays focused
on source parsing and editor intelligence.

Long-running workbook operations are cancelled by terminating the `vba-dev`
process from the extension. `vba-dev` owns cleanup for opened workbooks,
...
containing the CommonModules manifest and source files. `vba-dev` consumes
...
Marketplace packaging should not accidentally include the `vba-dev` source
tree. A future packaging decision can add a built `vba-dev.exe` as a
