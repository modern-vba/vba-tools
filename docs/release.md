...
standalone `vba-devtool` artifact.
...
- `bin/vba-devtool/win-x64/vba-devtool.exe` must be present.
- `tools/vba-devtool/**` must be absent from the VSIX file list.
- The bundled executable must answer `capabilities --format json` with the
  command contract required by the extension.
- `VbaDevTool.Cli.csproj` must publish a Windows x64 self-contained single-file
  executable.
...
## Standalone VbaDevTool artifact
...
`bin/vba-devtool/win-x64/` to the GitHub Release, either directly or as a
`vba-devtool-win-x64.zip` archive. The executable is the runnable command; keep
...
```powershell
bin/vba-devtool/win-x64/vba-devtool.exe --help
bin/vba-devtool/win-x64/vba-devtool.exe capabilities --format json
```
