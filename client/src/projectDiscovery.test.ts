...
test('ProjectManifest discovery walks upward from an active VBA source file', async () => {
  ...
  const existingFiles = new Set([path.join(projectRoot, 'vba-project.json')]);
...
test('ProjectManifest discovery treats vba-project.json itself as the manifest', async () => {
  const manifestPath = path.join('C:', 'work', 'BookProject', 'vba-project.json');
...
test('WorkbookBackedProject selection uses workspace candidates when the active file is ambiguous', async () => {
  ...
  const selected = await discoverWorkbookBackedProject({
    ...
    findProjectManifests: async () => [
      path.join(firstRoot, 'vba-project.json'),
      path.join(secondRoot, 'vba-project.json')
    ],
  ...
  assert.equal(selected?.manifestPath, path.join(secondRoot, 'vba-project.json'));
});
