...
export async function validateStagedReleaseAssets({
  ...
  try {
    ...
    const contract = JSON.parse(cliEntries.get('vba-dev-contract.json').toString('utf8'));
    assertCliContract(capabilities, contract, vbaDevVersion);
  } finally {
...
function assertCliContract(capabilities, contract, version) {
  if (
    capabilities?.toolVersion !== version ||
    capabilities?.contractVersion !== contract?.contractVersion
  ) {
    throw new Error('Staged vba-dev capabilities disagree with its version or command contract.');
  }
  for (const [commandName, schemaVersion] of Object.entries(
    contract.commandSchemaVersions ?? {}
  )) {
    if (capabilities.commands?.[commandName]?.outputSchemaVersion !== schemaVersion) {
      throw new Error(`Staged vba-dev ${commandName} schema disagrees with its command contract.`);
    }
  }
  if (
    contract.debugAdapterProtocolVersion !== undefined &&
    capabilities.debugAdapter?.protocolVersion !== contract.debugAdapterProtocolVersion
  ) {
    throw new Error('Staged vba-dev debug adapter protocol disagrees with its command contract.');
  }
}
