...
export function validateReleaseProtections({
  creationRule,
  immutableTagRule,
  environment,
  deploymentPolicies
}) {
  const expectedRef = 'refs/tags/vba-tools-v*';
  for (const [rule, name, types] of [
    [creationRule, 'authorized vba-tools release tag creation', ['creation']],
    [immutableTagRule, 'keep vba-tools release tags immutable', ['deletion', 'update']]
  ]) {
    if (rule?.name !== name || rule?.target !== 'tag' || rule?.enforcement !== 'active') {
      throw new Error(`Required active release tag ruleset is missing: ${name}.`);
    }
    if (
      JSON.stringify(rule.conditions?.ref_name?.include) !== JSON.stringify([expectedRef]) ||
      JSON.stringify((rule.rules ?? []).map((item) => item.type).sort()) !== JSON.stringify(types)
    ) {
      throw new Error(`Release tag ruleset has inconsistent scope or rules: ${name}.`);
    }
    if (rule.conditions?.ref_name?.exclude?.length !== 0) {
      throw new Error('Release tag rulesets must not contain excluded release tags.');
    }
  }
  if (
    environment?.name !== 'marketplace-release' ||
    environment.deployment_branch_policy?.protected_branches !== false ||
    environment.deployment_branch_policy?.custom_branch_policies !== true ||
    JSON.stringify((environment.protection_rules ?? []).map((rule) => rule.type)) !==
      JSON.stringify(['branch_policy']) ||
    deploymentPolicies?.total_count !== 1 ||
    JSON.stringify(deploymentPolicies?.branch_policies?.map(
      (policy) => ({ name: policy.name, type: policy.type })
    )) !== JSON.stringify([{ name: 'vba-tools-v*', type: 'tag' }])
  ) {
    throw new Error('Marketplace environment must accept only protected release tags without a second approval.');
  }
  return { releaseTagPattern: 'vba-tools-v*', environment: 'marketplace-release' };
}
...
export async function runReleaseWorkflowCommand(
  ...
  let result;
  if (parsed.command === 'validate-protections') {
    if (repository !== 'modern-vba/vba-tools') {
      throw new Error('Release protection preflight is limited to modern-vba/vba-tools.');
    }
    const prefix = `repos/${repository}`;
    const getApi = async (endpoint) => JSON.parse(
      (await runCommand('gh', ['api', endpoint], root)).stdout
    );
    const summaries = await getApi(`${prefix}/rulesets`);
    const findRule = (name) => {
      const matching = summaries.filter(
        (rule) => rule.name === name && rule.target === 'tag' && rule.enforcement === 'active'
      );
      if (matching.length !== 1 || !Number.isInteger(matching[0].id)) {
        throw new Error(`Required active release tag ruleset is missing or ambiguous: ${name}.`);
      }
      return matching[0];
    };
    const creationSummary = findRule('authorized vba-tools release tag creation');
    const immutableSummary = findRule('keep vba-tools release tags immutable');
    const [creationRule, immutableTagRule, environment, deploymentPolicies] =
      await Promise.all([
        getApi(`${prefix}/rulesets/${creationSummary.id}`),
        getApi(`${prefix}/rulesets/${immutableSummary.id}`),
        getApi(`${prefix}/environments/marketplace-release`),
        getApi(`${prefix}/environments/marketplace-release/deployment-branch-policies`)
      ]);
    result = validateReleaseProtections({
      creationRule,
      immutableTagRule,
      environment,
      deploymentPolicies
    });
  } else if (parsed.command === 'validate-tag') {
  ...
  } else if (parsed.command === 'validate-assets' || parsed.command === 'validate-assets-static') {
    result = await validateStagedReleaseAssets({
      ...
      sourceRoot: root,
      probeExecutables: parsed.command === 'validate-assets',
      runCommand
    });
...
function parseWorkflowCommandArguments(args) {
  ...
  const commandOptions = {
    ...
    'validate-assets-static': assetOptions(),
