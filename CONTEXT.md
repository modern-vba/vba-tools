...
**CallableSignature**:
The structured call shape for a callable `VbaDefinition` or `VbaProjectReferenceDefinition`. It includes the displayed signature label, ordered parameters, optional parameter metadata, parameter passing metadata, parameter type names, default values, return type names, and parameter documentation when that documentation is available from source comments or reference catalog metadata. When shown by Signature Help or as a callable hover declaration line, the primary signature label carries the callable kind (`Sub`, `Function`, `Property`, or `Event`), available return type, available parameter type metadata, and effective `ByRef` metadata, including implicit VBA `ByRef`, while `ByVal` is omitted even when explicit; property accessors are collapsed to `Property`, `ParamArray` is shown when available, array parameters keep their `()` marker, optional parameters are represented with brackets rather than the `Optional` keyword, and visibility modifiers and default values are omitted from the primary label. Reference catalog signatures follow the same display rules but only show metadata supplied by the catalog; missing passing, type, or default metadata is not inferred.
_Avoid_: parameter list, call text, method shape

**Hover**:
An editor feature that explains the `VbaDefinition` or `VbaProjectReferenceDefinition` under the cursor. For source-defined and reference-catalog callable definitions, hover shows the definition documentation and the same `CallableSignature` label used by Signature Help, but it does not expand per-parameter documentation or track an active `CallableParameter`.
_Avoid_: SignatureHelp, tooltip, parameter hover

**SignatureHelp**:
An editor feature that shows the `CallableSignature` for a resolved call site and tracks the active `CallableParameter`. It is distinct from hover: Signature Help explains the callable currently being invoked, while hover explains the definition under the cursor. When the signature label includes parameter type or passing metadata, the active parameter range covers the whole displayed parameter segment rather than only the parameter name.
_Avoid_: hover, tooltip, parameter hover

**DeclarationLabel**:
The editor-facing declaration summary for a `VbaDefinition` or `VbaProjectReferenceDefinition` when no richer `CallableSignature` label applies. It includes the declaration name, declared parameter names when applicable, and type metadata when available, without synthesizing implicit VBA default types that are not already represented. Callable and event hover declaration lines use the `CallableSignature` label so hover and Signature Help present the same parameter metadata. Constant, enum, and user-defined type labels include `Const`, `Enum`, or `Type`; an event without a represented `CallableSignature` may still fall back to an `Event` declaration label. Variable, parameter, enum member, and user-defined type member labels use the declaration form, such as `Name As Type`, without scope or category words, and array declarations keep the `()` marker after the name. Parameter labels include the declared type and effective `ByRef` passing metadata when relevant, including parameters without an explicit passing keyword; `ByVal` is omitted. `Static` and `WithEvents` are included when they apply, but visibility modifiers such as `Public`, `Private`, and `Friend` are omitted.
