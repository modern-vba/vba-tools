...
/// <summary>
/// Reads immutable project metadata from Open XML package bytes fixed by the caller.
/// </summary>
public sealed partial class VbaProjectPackageMetadataReader
{
    ...
    /// <summary>The maximum admitted package size, also usable at caller capture boundaries.</summary>
    public const int MaximumPackageLength = 512 * 1024 * 1024;
    ...
    public VbaProjectPackageMetadataReadResult Read(
        byte[] callerFixedPackageBytes,
        CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var result = ReadCore(callerFixedPackageBytes, cancellationToken);
        cancellationToken.ThrowIfCancellationRequested();
        return result;
    }

    private static VbaProjectPackageMetadataReadResult ReadCore(
        byte[] callerFixedPackageBytes,
        CancellationToken cancellationToken)
    {
        if (callerFixedPackageBytes is null)
        {
            return VbaProjectPackageMetadataReadResult.Failed(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackage,
                "The VBA package bytes are missing.");
        }

        if (callerFixedPackageBytes.Length is <= 0
            || callerFixedPackageBytes.Length > MaximumPackageLength)
        {
            return VbaProjectPackageMetadataReadResult.Failed(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackage,
                "The VBA package has an invalid or excessive length.");
        }
        ...
        try
        {
            ...
            cancellationToken.ThrowIfCancellationRequested();
            return VbaProjectPackageMetadataReadResult.Success(
                new VbaProjectPackageMetadata(
                    projectInformation.ProjectName,
                    projectInformation.CodePage,
                    projectInformation.SystemKind,
                    projectInformation.Constants,
                    VbaProjectPartContentIdentity.FromBytes(vbaProjectPart)));
        }
        catch (PackageMetadataFormatException exception)
        {
            return VbaProjectPackageMetadataReadResult.Failed(
                exception.Kind,
                exception.Message);
            ...
            or XmlException)
        {
            return VbaProjectPackageMetadataReadResult.Failed(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackage,
                $"The VBA bytes are not a supported Open XML package: {exception.Message}");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        if (archive.Entries.Count > MaximumArchiveEntryCount)
        {
            throw new PackageMetadataFormatException(
                ...
                "The VBA package contains too many parts.");
        }
        ...
        if (contentTypeEntries.Length != 1)
        {
            throw new PackageMetadataFormatException(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackageTopology,
                "The VBA package must contain exactly one [Content_Types].xml part.");
        }
        ...
        if (contentTypesEntry.Length is <= 0 or > MaximumContentTypesPartLength)
        {
            throw new PackageMetadataFormatException(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackageTopology,
                "The VBA content-types part has an invalid or excessive length.");
        }
        ...
        catch (XmlException exception)
        {
            throw new PackageMetadataFormatException(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackageTopology,
                $"The VBA content-types part is malformed: {exception.Message}",
                exception);
        }
        ...
        if (document.Root?.Name != ContentTypesNamespace + "Types")
        {
            throw new PackageMetadataFormatException(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackageTopology,
                "The VBA content type declarations are not in the OPC namespace.");
        }
                ...
                && element.Name != ContentTypesNamespace + "Override"))
        {
            throw new PackageMetadataFormatException(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackageTopology,
                "The VBA package contains a non-OPC content type declaration.");
        }
                     ...
                     element.Name == ContentTypesNamespace + "Default"))
        {
                ...
                || !defaults.TryAdd(extension, contentType))
            {
                throw new PackageMetadataFormatException(
                    VbaProjectPackageMetadataReadFailureKind.InvalidPackageTopology,
                    "The VBA package contains invalid or ambiguous default content types.");
            }
        }
                     ...
                     element.Name == ContentTypesNamespace + "Override"))
        {
                ...
                || !overrides.TryAdd(partName, contentType))
            {
                throw new PackageMetadataFormatException(
                    VbaProjectPackageMetadataReadFailureKind.InvalidPackageTopology,
                    "The VBA package contains invalid or ambiguous content type overrides.");
            }
        }
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            throw new PackageMetadataFormatException(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackageTopology,
                "The VBA package must contain exactly one xl/vbaProject.bin VBA project part.");
        }
        ...
        if (part.Length is <= 0 or > MaximumVbaProjectPartLength)
        {
            throw new PackageMetadataFormatException(
                VbaProjectPackageMetadataReadFailureKind.InvalidVbaProjectPart,
                "The VBA project part has an invalid or excessive length.");
        }
        ...
        CancellationToken cancellationToken)
    {
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            throw new PackageMetadataFormatException(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackageTopology,
                "The VBA package must contain exactly one macro-enabled xl/workbook.xml part.");
        }
        ...
        if (relationshipParts.Length != 1)
        {
            throw new PackageMetadataFormatException(
                VbaProjectPackageMetadataReadFailureKind.InvalidPackageTopology,
                "The VBA package must contain exactly one workbook relationships part.");
        }
    ...
    private static byte[] ReadBoundedEntry(
        ...
        int maximumLength,
        VbaProjectPackageMetadataReadFailureKind failureKind,
        string description,
        CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            if (read > remaining)
            {
                throw new PackageMetadataFormatException(
                    failureKind,
                    $"The VBA {description} part exceeds its configured bound.");
            }
        ...
        if (buffer.Length <= 0)
        {
            throw new PackageMetadataFormatException(
                failureKind,
                $"The VBA {description} part is empty.");
        }
    ...
    private static byte[] ReadDirectoryStream(byte[] vbaProjectPart)
    {
        ...
        catch (PackageMetadataFormatException)
        {
            throw;
        }
    ...
    private sealed record ProjectInformation(
        string ProjectName,
        int CodePage,
        VbaProjectSystemKind SystemKind,
        IReadOnlyDictionary<string, short> Constants)
    {
        public static ProjectInformation Read(ReadOnlySpan<byte> directory)
        {
            try
            {
                ...
                IReadOnlyDictionary<string, short> constants = new Dictionary<string, short>();
                ...
                return new ProjectInformation(
                    projectName, codePage, (VbaProjectSystemKind)systemKind, constants);
            }
            catch (PackageMetadataFormatException)
            {
                throw;
        ...
        private static IReadOnlyDictionary<string, short> ReadConstants(
            Encoding encoding,
            ReadOnlySpan<byte> mbcsBytes,
            ReadOnlySpan<byte> unicodeBytes)
            => ReadConstantsText(ValidateEquivalentStrings(
                encoding, mbcsBytes, unicodeBytes, "PROJECTCONSTANTS"));

        private static IReadOnlyDictionary<string, short> ReadConstantsText(string text)
        {
            ...
            foreach (var item in text.Split(" : ", StringSplitOptions.None))
            {
                ...
                if (!constants.TryAdd(assignment[0], value))
                {
                    throw new InvalidDataException(
        ...
        private static string ValidateEquivalentStrings(
            Encoding encoding,
            ReadOnlySpan<byte> mbcsBytes,
            ReadOnlySpan<byte> unicodeBytes,
            string recordName)
    ...
    private sealed class PackageMetadataFormatException
        : Exception
    {
        public PackageMetadataFormatException(
        ...
        public VbaProjectPackageMetadataReadFailureKind Kind { get; }
    }
}
