...
public sealed partial class VbaProjectPackageMetadataReader
{
    ...
    /// </summary>
    private static class MsOvbaCompression
    {
            ...
            while (compressedCurrent < compressedContainer.Length)
            {
                ...
                if ((header & CompressedChunkFlag) != 0)
                {
                    ...
                    DecompressTokenSequences(
                        ...
                        maximumOutputLength,
                        cancellationToken);
                    if (chunkEnd < compressedContainer.Length
                        && output.Count - decompressedChunkStart
                        != DecompressedChunkLength)
                    {
                        throw new InvalidDataException(
                            "A non-final MS-OVBA compressed chunk must decompress to exactly 4096 bytes.");
                    }
