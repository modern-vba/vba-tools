...
public sealed class AutomationDesktopIsolationWindowsExcelIntegrationTests
{
    ...
    public async Task SharedProductionAutomationKeepsRepresentativeWorkOffCallerDesktop()
    {
        ...
        using var importSourceSet = VbeImportSourceSet.Create(new WindowsExactFileSystemObjectOwnershipFactory(), new VbaSourceAdmission(() => ActiveWindowsAnsiCodePage.Get()).AdmitExplicitImport(Path.GetDirectoryName(sourcePath)!, CancellationToken.None));
