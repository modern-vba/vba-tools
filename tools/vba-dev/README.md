...
An incomplete build, publish, or build-before-test source analysis automatically
saves local diagnostic evidence when a non-cancellation processing exception was
captured. The command reports the absolute JSON path on stderr; the VS Code
extension retains that line in VBA Tools Output. The existing `sourceAnalysis`
schema `3.0` failure objects additionally expose `phase`, `exceptionType`, and
bounded `exception` details. Existing messages and completeness semantics remain
unchanged. See the [investigation guide](../../docs/source-analysis-failure-evidence.md)
for retention, privacy, capture limitations, and safe follow-up steps.
