...
/// </summary>
internal sealed class WindowsExactFileSystemObjectOwnership : ExactFileSystemObjectOwnership
{
    ...
    private readonly Dictionary<FileStream, LeaseMarkerCreation> leaseMarkerCreations =
        new(ReferenceEqualityComparer.Instance);
    private bool disposed;

    private WindowsExactFileSystemObjectOwnership()
    {
    }
    ...
    /// </summary>
    internal static WindowsExactFileSystemObjectOwnership Open()
    {
        ...
        return new WindowsExactFileSystemObjectOwnership();
    }

    /// <summary>
    /// Creates a lease marker without releasing its live ownership handle.
    /// The optional override is the existing trusted create-only test seam.
    /// The caller owns and releases the returned stream.
    /// </summary>
    internal FileStream CreateOnlyLeaseMarker(
        string route,
        bool deleteOnClose,
        Func<string, FileStream>? createOwnerStreamOverride = null)
    {
        ...
        var stream = createOwnerStreamOverride?.Invoke(absoluteRoute)
            ?? new FileStream(
                absoluteRoute,
                FileMode.CreateNew,
        ...
        try
        {
            leaseMarkerCreations.Add(
                stream,
                new LeaseMarkerCreation(absoluteRoute, stream.SafeFileHandle));
            return stream;
        }
        catch
        {
    ...
    /// <summary>
    /// Issues a receipt from this session's still-open creation handle after
    /// the lease provider has written and flushed its informational metadata.
    /// </summary>
    internal FileReceipt CaptureCreatedLeaseMarker(FileStream creationStream)
    {
        ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(creationStream);
        if (!leaseMarkerCreations.TryGetValue(creationStream, out var creation)
            || creation.Receipt is not null
            || creation.Handle.IsClosed)
        {
            throw new ArgumentException(
                "The lease marker stream is not an uncaptured creation in this ownership session.",
                nameof(creationStream));
        }

        SafeFileHandle? anchor = null;
        try
        {
            var information = ReadObjectInformation(creation.Handle, creation.Route);
            RequireOrdinarySingleLinkFile(information, expectedLength: null, creation.Route);
            var hash = ComputeSha256(creation.Handle, information.Length);
            if (!MatchesFixedFileState(
                    creation.Handle, creation.Route, information.Identity, information.Length, hash,
                    expectedDeletePending: false, static links => links == 1))
            {
                throw new IOException(
                    $"The created lease marker changed during capture: '{creation.Route}'.");
            }

            anchor = OpenAnchor(creation.Route);
            if (!MatchesIssuedFile(
                    ReadObjectInformation(anchor, creation.Route), information.Identity, information.Length))
            {
                throw new IOException(
                    $"The created lease marker route changed during capture: '{creation.Route}'.");
            }

            var receipt = new FileReceipt(new OwnedFileReceiptState(new FileReceiptProof(
                this, creation.Route, information.Identity, information.Length, hash, anchor)));
    ...
    /// <summary>
    /// Finishes an already-released lease's DeleteOnClose lifetime before the
    /// provider's release callback. It never sets a deletion disposition.
    /// False leaves receipt-based cleanup and its outcome policy to the caller.
    /// </summary>
    internal bool CompleteReleasedLeaseMarker(FileStream creationStream)
    {
        ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(creationStream);
        if (!leaseMarkerCreations.TryGetValue(creationStream, out var creation)
            || creation.Receipt is not { } receipt
            || !creation.Handle.IsClosed)
        {
            throw new ArgumentException(
                "The lease marker stream is not a released captured creation in this ownership session.",
                nameof(creationStream));
        }

        RequireOwned(receipt);
        leaseMarkerCreations.Remove(creationStream);
        try
        {
            var authority = (IFileReceiptAuthority)receipt.State;
            var information = ReadObjectInformation(authority.Anchor!, receipt.Route);
            if (!MatchesAnchoredFile(receipt, information)
                || information.NumberOfLinks > 1
                || (!information.DeletePending && information.NumberOfLinks != 0))
            {
                return false;
            }
    ...
    /// </summary>
    internal override DirectoryReceipt? TryCreateOnlyDirectory(
        string parentRoute,
        string childName)
    {
        ThrowIfDisposed();
        ...
        try
        {
            ...
            var receipt = new DirectoryReceipt(new OwnedDirectoryReceiptState(new DirectoryReceiptProof(
                this,
                route,
    ...
    /// </summary>
    internal override FileReceipt CreateOnlyFile(
        ...
        Action<string>? onDirectoryProofComplete = null)
    {
        ...
        var directoryAuthority = (IDirectoryReceiptAuthority)directory.State;
        ...
        Exception originalFailure)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    retainedReceipt = new FileReceipt(new OwnedFileReceiptState(new FileReceiptProof(
    ...
    /// </summary>
    internal override PendingFileCapture CapturePendingSavedFile(string route)
    {
        ThrowIfDisposed();
        ArgumentException.ThrowIfNullOrWhiteSpace(route);
        var absoluteRoute = Path.GetFullPath(route);
        ...
        try
        {
            ...
            var pending = new PendingFileCapture(new OwnedPendingFileCaptureState(new PendingFileCaptureProof(
    ...
    /// </summary>
    internal override StableCaptureCompletion CompleteStableCapture(PendingFileCapture pending)
    {
        ThrowIfDisposed();
        ArgumentNullException.ThrowIfNull(pending);
        var state = ((IOwnedPendingFileCaptureState)pending.State).State;
    ...
    /// </summary>
    internal override StableFileCapture CaptureTrustedStableFile(string route)
        => CaptureTrustedStableFileCore(route, GenericRead);

    /// <summary>
    /// Preserves lease cleanup's read/delete sharing admission while capturing
    /// a trusted stale marker through the ordinary stable-capture proof.
    /// </summary>
    internal StableFileCapture CaptureTrustedStaleLeaseMarker(string route)
        => CaptureTrustedStableFileCore(route, GenericRead | DeleteAccess);

    private StableFileCapture CaptureTrustedStableFileCore(string route, uint desiredAccess)
    {
        ThrowIfDisposed();
        ArgumentException.ThrowIfNullOrWhiteSpace(route);
        var absoluteRoute = Path.GetFullPath(route);
    ...
    /// </summary>
    internal override void ReleaseCreationFence(DirectoryReceipt directory)
    {
        ThrowIfDisposed();
        RequireOwned(directory);
    ...
    /// </summary>
    internal override ObservationResult Observe(FileReceipt receipt, Action<string>? onProofComplete = null)
    {
        ThrowIfDisposed();
        RequireOwned(receipt);
        ...
        try
        {
            var anchorInformation = ReadObjectInformation(
                ((IFileReceiptAuthority)receipt.State).Anchor!, receipt.Route);
    ...
    /// </summary>
    internal override ObservationResult Observe(DirectoryReceipt receipt)
    {
        ThrowIfDisposed();
        RequireOwned(receipt);
        try
        {
            var anchorInformation = ReadObjectInformation(
                ((IDirectoryReceiptAuthority)receipt.State).Anchor!, receipt.Route);
    ...
    /// supplied receipt.
    /// </summary>
    internal override DeletionResult TryDelete(
        FileReceipt receipt,
    ...
    /// supplied receipt.
    /// </summary>
    internal override DeletionResult TryDeleteEmpty(
        ...
        Action<string>? onProofComplete = null)
    {
        ...
        try
        {
            var authority = (IDirectoryReceiptAuthority)receipt.State;
            var anchorInformation = ReadObjectInformation(
                authority.Anchor!,
                receipt.Route);
            if (!MatchesAnchoredDirectory(receipt, anchorInformation))
    ...
    public override void Dispose()
    {
        ...
        try
        {
            ...
            {
                ((IOwnedPendingFileCaptureState)pending.State).State.Handle.Dispose();
            }
        }
        finally
        {
    ...
    private void RequireOwned(FileReceipt receipt)
    {
        ...
        if (receipt.State is not IFileReceiptAuthority authority
            || !ReferenceEquals(authority.Owner, this)
            || !fileReceipts.Contains(receipt))
        {
            throw new InvalidOperationException(
    ...
    {
        ArgumentNullException.ThrowIfNull(receipt);
        if (receipt.State is not IDirectoryReceiptAuthority authority
            || !ReferenceEquals(authority.Owner, this)
            || !directoryReceipts.Contains(receipt))
        {
            throw new InvalidOperationException(
    ...
    {
        ((IFileReceiptAuthority)receipt.State).Retire()?.Dispose();
        fileReceipts.Remove(receipt);
    }

    private void Retire(DirectoryReceipt receipt)
    {
        var handles = ((IDirectoryReceiptAuthority)receipt.State).Retire();
    ...
    private static void ReleaseCreationFenceCore(DirectoryReceipt receipt)
        => ((IDirectoryReceiptAuthority)receipt.State).ReleaseCreationFence();
    ...
    {
        WindowsExactFileSystemObjectOwnership Owner { get; }
    ...
    /// </summary>
    private sealed class OwnedFileReceiptState : IFileReceiptAuthority, ExactFileSystemObjectOwnership.IFileReceiptState
    {
        private readonly WindowsExactFileSystemObjectOwnership owner;
        ...
        internal OwnedFileReceiptState(object provenState)
        {
        ...
        string ExactFileSystemObjectOwnership.IFileReceiptState.Route => Route;

        WindowsExactFileSystemObjectOwnership IFileReceiptAuthority.Owner => owner;
    ...
    /// </summary>
    private sealed class OwnedDirectoryReceiptState : IDirectoryReceiptAuthority, ExactFileSystemObjectOwnership.IDirectoryReceiptState
    {
        private readonly WindowsExactFileSystemObjectOwnership owner;
        private readonly ObjectIdentity identity;
        ...
        internal OwnedDirectoryReceiptState(object provenState)
        {
        ...
        string ExactFileSystemObjectOwnership.IDirectoryReceiptState.Route => Route;

        WindowsExactFileSystemObjectOwnership IDirectoryReceiptAuthority.Owner => owner;
    ...
    private interface IOwnedPendingFileCaptureState
    {
    ...
    /// </summary>
    private sealed class OwnedPendingFileCaptureState : IOwnedPendingFileCaptureState, ExactFileSystemObjectOwnership.IPendingFileCaptureState
    {
        private readonly PendingFileCaptureProof state;

        internal OwnedPendingFileCaptureState(object provenState)
        {
        ...
        string ExactFileSystemObjectOwnership.IPendingFileCaptureState.Route => Route;

        byte[] ExactFileSystemObjectOwnership.IPendingFileCaptureState.Bytes => state.Bytes.ToArray();

        internal byte[] Bytes => state.Bytes.ToArray();

        PendingFileCaptureProof IOwnedPendingFileCaptureState.State => state;
    }
    ...
    private sealed record FileReceiptProof(
        WindowsExactFileSystemObjectOwnership Owner,
        string Route,
        ObjectIdentity Identity,
        long Length,
        byte[] Sha256,
    ...
    private sealed record StableFileCaptureProof(
        FileReceipt Receipt,
        byte[] Bytes) : ExactFileSystemObjectOwnership.IStableFileCaptureState;
    ...
    private sealed record LeaseMarkerCreation(string Route, SafeFileHandle Handle)
    {
        internal FileReceipt? Receipt { get; set; }
    }
