...
public static class ToolingCommandCatalog
{
    public static IReadOnlyList<ToolingCommandContract> CreateDefaultContracts()
    {
        ...
        [
            ...
            new("import", "Run a path-only import of VBA sources into an existing workbook; unlike build, it does not use project.json.", "[options]", [Option("--from", "Source directory containing .bas, .cls, and .frm files.", "<dir>"), Option("--to", "Existing workbook file to update in place.", "<path>")], 75, ToolingCommandContextPolicy.None),
