...
internal interface IFileSystemPathIdentityResolver
{
    FileSystemPathIdentity Resolve(string path);
}

internal sealed record FileSystemPathIdentity(
...
internal readonly record struct FileSystemObjectIdentity(
    uint VolumeSerialNumber,
    ulong FileIndex);

internal sealed class FileSystemPathIdentityResolver : IFileSystemPathIdentityResolver
{
...
internal sealed class BuildSourceSnapshotOutputSafetyValidator
{
        ...
        string outputPath)
    {
        ...
        if (IsSameOrDescendant(
                outputIdentity.CanonicalPath,
                snapshotIdentity.CanonicalPath))
        {
        ...
        foreach (var (documentName, document) in context.Manifest.Documents)
        {
            ...
            if (IsSameOrDescendant(
                    outputIdentity.CanonicalPath,
                    sourceSetIdentity.CanonicalPath))
            {
                throw new InvalidOperationException(
        ...
        string outputPath)
    {
        var sameCanonicalPath = Path.TrimEndingDirectorySeparator(
                outputIdentity.CanonicalPath)
            .Equals(
                Path.TrimEndingDirectorySeparator(protectedIdentity.CanonicalPath),
                StringComparison.OrdinalIgnoreCase);
        var sameExistingObject = outputIdentity.ObjectIdentity is not null
            && protectedIdentity.ObjectIdentity is not null
            && outputIdentity.ObjectIdentity == protectedIdentity.ObjectIdentity;
        if (sameCanonicalPath || sameExistingObject)
        {
            throw new InvalidOperationException(
