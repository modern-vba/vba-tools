...
public sealed class DebugConditionalCompilationPreflight
{
        ...
        VbaConditionalCompilationEnvironment environment)
    {
        ...
        {
            var evaluation = evaluations[targetSource.Source.SourceUri];
        ...
        foreach (var breakpoint in conditionalBreakpoints)
        {
            var evaluation = evaluations[breakpointSources[breakpoint].Source.SourceUri];
    ...
    {
        if (evaluations.TryGetValue(source.Source.SourceUri, out var existing))
        {
            return existing;
        }

        var evaluation = VbaConditionalCompilationEvaluator.Evaluate(source.Tree, environment);
        evaluations.Add(source.Source.SourceUri, evaluation);
        return evaluation;
    }
