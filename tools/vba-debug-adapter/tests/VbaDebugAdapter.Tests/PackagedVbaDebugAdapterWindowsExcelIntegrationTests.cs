...
public sealed class PackagedVbaDebugAdapterWindowsExcelIntegrationTests
{
    private const string BundledVbaDevPath = "bin/vba-dev/win-x64/vba-dev.exe";
    private const string BundledDebugAdapterPath =
    ...
    [Fact]
    public void PersistentArtifactBaselineIncludesATemplateOutsideTheSourceSet()
    {
        using var temp = TempDirectory.Create();
        var sourceSetPath = Path.Combine(temp.Path, "source");
        var templatePath = Path.Combine(temp.Path, "template", "Book1.xlsm");
        var binPath = Path.Combine(temp.Path, "bin", "Book1.xlsm");
        var publishPath = Path.Combine(temp.Path, "publish", "Book1.xlsm");
        foreach (var filePath in new[] { templatePath, binPath, publishPath })
        {
            Directory.CreateDirectory(Path.GetDirectoryName(filePath)!);
            File.WriteAllBytes(filePath, [0x50, 0x4b]);
        }
        Directory.CreateDirectory(sourceSetPath);
        File.WriteAllText(Path.Combine(sourceSetPath, "Module1.bas"), "Option Explicit");

        var artifacts = CapturePersistentArtifacts(new ProjectDocumentPaths(
            sourceSetPath,
            templatePath,
            binPath,
            publishPath,
        ...
        Assert.True(artifacts.ContainsKey(Path.GetFullPath(templatePath)));
    }
    ...
    public async Task PackagedVsCodeAssetsCompleteTheNativeBreakpointWorkflowAndReportTerminalLifecycle()
    {
        ...
        try
        {
            ...
            Assert.True(
                persistentArtifacts.ContainsKey(Path.GetFullPath(documentPaths.TemplatePath)),
            ...
            Assert.False(File.ReadAllBytes(sourcePath).AsSpan().SequenceEqual(sourceBytes));
            ...
            Assert.Equal(
                documentPaths.WorkbookFileName,
                Path.GetFileName(launchReport.WorkbookPath),
                ignoreCase: true);
    ...
    private static PackagedDebugAssets ResolvePackagedDebugAssets(string repositoryRoot)
    {
        ...
        var runtimes = manifest.RootElement.GetProperty("runtimes");
        var vbaDevPath = ResolveBundledExecutable(
            repositoryRoot,
            runtimes,
            "vbaDev",
            BundledVbaDevPath);
        var debugAdapterPath = ResolveBundledExecutable(
            repositoryRoot,
            runtimes,
            "vbaDebugAdapter",
            BundledDebugAdapterPath);
        Assert.False(
            string.Equals(vbaDevPath, debugAdapterPath, StringComparison.OrdinalIgnoreCase));
        return new PackagedDebugAssets(vbaDevPath, debugAdapterPath);
    }
        ...
        string expectedRelativePath)
    {
        ...
        Assert.True(
            ...
            $"The bundled {runtimeName} manifest path must remain inside the repository root.");
        Assert.True(
    ...
    {
        var actual = CapturePersistentArtifacts(paths);
        Assert.Equal(
        ...
        {
            Assert.True(
                actual[path].AsSpan().SequenceEqual(bytes),
                $"Persistent debug artifact changed: {path}");
        }
    }

    private static string CreatePersistentSource()
        => string.Join(
            "\r\n",
            ...
            "    Err.Raise 5, \"PackagedAdapterTracer\", \"Persistent source must not run.\"",
            "End Sub",
            string.Empty);
    ...
    private static async Task<DebugLaunchReport> WaitForLaunchReportAsync(
        string path,
        TimeSpan timeout)
    {
        ...
        try
        {
            while (true)
            {
                if (File.Exists(path))
                {
                    var fields = File.ReadAllText(path).Trim().Split('|', 2);
                    if (fields.Length == 2 &&
                        long.TryParse(fields[0], out var handle) &&
                        handle != 0 &&
                        Path.IsPathFullyQualified(fields[1]))
                    {
                        return new DebugLaunchReport(new nint(handle), fields[1]);
                    }
                }
        ...
        catch (OperationCanceledException) when (cancellation.IsCancellationRequested)
        {
            throw new TimeoutException(
                $"The packaged debug target did not report its Excel window and workbook within {timeout}.");
        }
    }
    ...
    private static Task<T> RunInStaAsync<T>(Func<T> operation)
    {
        ...
        {
            IsBackground = true,
            Name = "Packaged VBA debug adapter smoke COM observer"
        };
    ...
    private sealed class PackagedDebugAdapterProcess : IAsyncDisposable
    {
        ...
        public static PackagedDebugAdapterProcess Start(
            ...
            string vbaDevExecutablePath,
            string workingDirectory,
            string sessionId)
        {
            var startInfo = new ProcessStartInfo
            {
                FileName = debugAdapterExecutablePath,
                WorkingDirectory = workingDirectory,
                UseShellExecute = false,
                RedirectStandardInput = true,
                RedirectStandardOutput = true,
            ...
            startInfo.ArgumentList.Add("--vba-dev");
            startInfo.ArgumentList.Add(Path.GetFullPath(vbaDevExecutablePath));
            startInfo.ArgumentList.Add("--session");
            startInfo.ArgumentList.Add(sessionId);
            var process = Process.Start(startInfo)
                ?? throw new InvalidOperationException(
                    "The packaged standalone VBA debug adapter process did not start.");
    ...
    private sealed record PackagedDebugAssets(
        string VbaDevExecutablePath,
        string DebugAdapterExecutablePath);
    ...
    private sealed record DebugLaunchReport(
        nint ExcelWindowHandle,
        string WorkbookPath);
